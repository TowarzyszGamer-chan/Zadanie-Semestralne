...
$rok = trim($_POST["rok_zalozenia"] ?? "");
$czlonkowie = trim($_POST["obecni_czlonkowie"] ?? "");

if ($nazwa === "") exit("Nazwa jest wymagana.");
if (!is_numeric($rok) || $rok < 1800 || $rok > date("Y")) exit("Nieprawidłowy rok.");
...
$stmt = $pdo->prepare("
    UPDATE zespoly
    SET nazwa=?, gatunek=?, kraj=?, rok_zalozenia=?, obecni_czlonkowie=?, zdjecie=?
    WHERE id=?
");
...
header("Location: zespol_details.php?id=" . $id);
exit;
